...
class Point(namedtuple("_Point", ("x", "y"))):
    ...
    def add(self, other):
        ...
        if self.x == 0 and self.y == 0:
            return other
        (u1, v1) = (FQ(self.x), FQ(self.y))
        (u2, v2) = (FQ(other.x), FQ(other.y))
        # print("u3 up = ", (u1 * v2 + v1 * u2)-21888242871839275222246405745257275088548364400416034343698204186575808495617)
        # print("u3 down = ", (FQ.one() + JUBJUB_D * u1 * u2 * v1 * v2) - 21888242871839275222246405745257275088548364400416034343698204186575808495617)
        u3 = (u1 * v2 + v1 * u2) / (FQ.one() + JUBJUB_D * u1 * u2 * v1 * v2)
        # print("u3 up / u3 down = ", u3)
        v3 = (v1 * v2 - FQ(JUBJUB_A) * u1 * u2) / (FQ.one() - FQ(JUBJUB_D) * u1 * u2 * v1 * v2)
        return Point(FQ(u3), FQ(v3))
    ...
    def mult(self, scalar):
        ...
        a = self.infinity()
        i = 0
        # scalar = scalar % 21888242871839275222246405745257275088548364400416034343698204186575808495617
        while scalar != 0:
            if (scalar & 1) != 0:
                a = a.add(p)
            p = p.double()
    ...
    def neg(self):
        ...
        """
        return Point(FQ(-self.x), FQ(self.y))
